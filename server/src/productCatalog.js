// Shared skincare product catalog data, used both by the one-off seed script
// (src/seed.js) and by the server's own startup auto-seed (see index.js) so a
// fresh MongoDB database is never left with an empty catalog.
...
export async function ensureProductCatalog() {
  ...
  if (count === 0) {
    ...
    console.log(`[db] Product catalog was empty — seeded ${PRODUCTS.length} products.`);
  }
}
